    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response, function (index, item) {
          $('.cards').append(
            '<div class="col s6"><div class="card" id="resource" data-id="' + item._id + '"><div class="card-image waves-effect waves-block waves-light"><img class="activator" style="height: 220px" src="' + item.thumbnail + '"></div><div class="card-content"><span class="card-title activator grey-text text-darken-4 line-clamp" style="line-height: 1.5;"><i class="material-icons right">more_vert</i>' + item.title + '</span><br><br><div class="chip">' + item.tags[0] + '</div><div class="chip">' + item.tags[1] + '<!-- <i class="material-icons">close</i> --></div></div><div class="card-reveal"><span class="card-title grey-text text-darken-4"><i class="material-icons right">close</i>' + item.title + '</span><p>' + item.description + '</p><br><div class="chip">' + item.tags[0] + '<i class="material-icons" id="delete-tag">close</i></div><div class="chip">' + item.tags[1] + '<i class="material-icons" id="delete-tag">close</i></div>' +
            '<br><br><button class="btn waves-effect waves-light" type="submit" name="action" id="submit" data-id="' + item._id + '">Delete<i class="material-icons"  id="delete-resource">delete</i></button></div>' +
